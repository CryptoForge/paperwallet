...
pub fn save_to_pdf(addresses: &str, filename: &str) -> Result<(), String> {
    let (doc, page1, layer1) = PdfDocument::new("Zero Paper Wallet", Mm(210.0), Mm(297.0), "Layer 1");
	...
	// Set title
	current_layer.use_text("Zero Paper Waller", 32.0, Mm(37.0), Mm(277.0), &font_bold);
...
fn add_address_to_page(current_layer: &PdfLayerReference, font: &IndirectFontRef, font_bold: &IndirectFontRef, address: &str, d1: &str, d2: &str, d3: &str, d4: &str, d5: &str, pos: u32) {
    ...
    current_layer.use_text("Zer Receiving Address", 14.0, Mm(55.0), Mm(ypos+27.5), &font_bold);
    ...
    current_layer.use_text("Zer Diversified Addresses ", 12.0, Mm(55.0), Mm(ypos+6.0), &font_bold);
